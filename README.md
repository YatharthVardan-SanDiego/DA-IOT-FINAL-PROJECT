...
- **EDA** (`eda.ipynb`) — Comprehensive exploratory data analysis covering class distribution, protocol/service patterns, TCP flag signatures, correlation structure, PCA, and feature distributions.
- **Model 1** — A deep neural network (DNN) that classifies each network flow as one of 12 attack or normal traffic types.
...
DA-IOT-FINAL-PROJECT/
│
├── eda.ipynb                              # Exploratory Data Analysis notebook (Yatharth Vardan)
│
├── models/
...
|---|---|
| `eda.ipynb` | Jupyter notebook with full EDA: class distribution, protocol/TCP flag analysis, correlation, PCA |
...
The EDA notebook (`eda.ipynb`) performs comprehensive analysis of the RT-IoT2022 dataset across 15 sections. It was authored by Yatharth Vardan and generates the following visualisation outputs:

| Output File | Description |
|---|---|
| `attack_distribution.png` | Bar chart and pie chart of class distribution across 12 attack types |
| `service_analysis.png` | Service distribution and service-by-attack-type heatmap |
| `protocol_analysis.png` | Protocol pie chart and protocol-usage stacked bar by attack type |
| `correlation_heatmap.png` | Triangular correlation heatmap of 21 key network features |
| `feature_boxplots.png` | Boxplots of 6 key features (flow duration, packet counts, SYN flags) across attack types |
| `mean_features_heatmap.png` | Normalized mean feature values heatmap by attack type |
| `tcp_flags_analysis.png` | SYN/ACK, FIN/RST, and PSH flag comparisons plus full flag heatmap |
| `port_analysis.png` | Top destination ports and most targeted port per attack type |
| `violin_plots.png` | Violin plots of flow duration, down/up ratio, packets/sec, forward packets |
| `attack_categories.png` | 4-category grouping pie chart and stacked bar by sub-type |
| `radar_chart.png` | Radar chart comparing 4 attack categories across 7 normalized features |
| `feature_distributions.png` | Density histograms of 6 features by attack type |
| `pca_visualization.png` | 2D PCA projection coloured by attack type and attack category |
| `scatter_analysis.png` | Scatter plots of forward vs backward packets, flow duration vs rate, SYN vs ACK |

### Key EDA Findings

| Finding | Detail |
|---|---|
| **No missing values** | All 10,464,945 cells populated; 0 duplicate rows |
| **Class imbalance ratio** | 3,380.68:1 (DOS_SYN_Hping: 76.89% vs NMAP_FIN_SCAN: 0.02%) |
| **Protocols** | 3 protocols: TCP (dominant), UDP, ICMP |
| **Services** | 10 service labels; `mqtt` and `-` account for the majority |
| **Most discriminative features** | `flow_SYN_flag_count`, `flow_duration`, `flow_pkts_per_sec` |
| **PCA separation** | DOS/DDoS and IoT device traffic form compact, well-separated clusters |
| **TCP flag signatures** | DOS_SYN_Hping: high SYN; Slowloris: high ACK; NMAP_FIN/XMAS: non-zero FIN; ARP: near-zero TCP flags |
| **Flow duration extremes** | NMAP flows: median ~4 µs; Slowloris: up to 21,728 s (6 hrs) |

### How to Run EDA

```bash
# Install additional dependency if not already installed
pip install scikit-learn

# Launch Jupyter and open eda.ipynb
jupyter notebook eda.ipynb
```

The notebook reads the dataset from `data.txt` (comma-separated). If using the `ucimlrepo` loader instead, export the dataset to `data.txt` or adapt the loading cell.
